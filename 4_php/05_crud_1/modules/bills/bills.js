...
var bills = (function bills() {
     ...
     * @param {string} mode - soit update, soit create
          ...
          // si l'utilisateur à rempli les champs ci-dessus
            ...
            xhr.onload = function getServerResponse() {
                ...
                if (mode === "create") {
                  ...
                  // ag@todo => gérer l'UI pour indiquer si erreur à l'update
                  // let status = Boolean(JSON.parse(this.response));
    ...
    function checkAllBills() {
      const boxes = tablerBills.querySelectorAll("td.delete input[type=checkbox]");
    ...
    function deleteBills() {
        ...
        if (checked.length) {
            ...
            checked.forEach(function(c) {
                ...
                idsBills.push(idBill); // idBills est accessible dans tous le scope du module bills
            });
     ...
     * Widget d'affichage du tableur.
     Un message indique à l'utilisateur si aucune facture n'est disponible
     *
    ...
    function displayTabler(mode) {
      const tabler = document.getElementById("tabler_bills");
        if (mode === "off") {
            // on masque le widget avec CSS
            tabler.classList.add("is-hidden");
        } else {
            // on affiche le widget avec CSS
            tabler.classList.remove("is-hidden");
        }
    }
     ...
     */
    function observer() {
        /* sélection des éléments html importants pour le module bills */
        ...
        const checkAllBillsBtn = document.getElementById("check_bills");
        ...
        if (checkAllBillsBtn) checkAllBillsBtn.onclick = checkAllBills;
    ...
    function openForm(billToEdit) {
        ...
        if (formBill.classList.contains("is-active")) {
            ...
            title.textContent = `Facture pour ${ activeUser.name }
            ${ activeUser.lastname }`;
    ...
    function paramCreateForm(evt) {
        ...
        /* on garde en mémoire dans une variable accessible dans tous le module,
        l'id de l'user que l'on va facturer */
        ...
        xhr.onload = function getUserFromServer() {
            // appel ajax a été effectué, on accède à la réponse echo par le serveur
    ...
    function updateBillInDOMList(createdAt, total) {
      ...
      td.textContent = total;
        // on récupère le td contenant la date + mise à jour
